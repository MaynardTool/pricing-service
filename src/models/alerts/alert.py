...
class Alert:
    ...
    def send(self):
        return requests.post(
            ...
            data={
                ...
                'to': self.user_email,
                'subject': 'Price list reach for {}'.format(self.item.name),
                'text': 'We have found a deal ({}). To navigate to the alert, visit {}'.format(self.item.url,
                                                                                               'http://www.pricing.edison.com/alerts/{}'.format(
                                                                                                   self._id))
            }
        )
    ...
    def save_to_mongo(self):
        Database.update(AlertConstants.COLLECTIONS, {'_id': self._id}, self.json())
    ...
    def delete(self):
        Database.remove(AlertConstants.COLLECTIONS, {'_id': self._id})
